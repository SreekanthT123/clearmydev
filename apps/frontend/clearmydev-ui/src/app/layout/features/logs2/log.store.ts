import {
  patchState,
  signalStore,
  withMethods,
  withProps,
  withState,
} from '@ngrx/signals';
import { inject } from '@angular/core';
import { ExplainLogResponse } from './log.model';
import { LogApi } from './log.service';
import { UsageService } from '../../../core/usage/usage.service';

export const LogStore = signalStore(
  withState({
    loading: false,
    result: null as ExplainLogResponse | null,
    error: null as string | null,
  }),
  withProps(() => ({
    _logService: inject(LogApi),
    _usageService: inject(UsageService),
  })),
  withMethods((store) => {
    return {
      explain(logs: string, context: string) {
        if (!logs.trim()) return;

        patchState(store, {
          loading: true,
          error: null,
          result: null,
        });

        store._logService.explain({ logs, context }).subscribe({
          next: (res) => {
            if (!res.success) {
              patchState(store, {
                loading: false,
                error: res.error || 'Something went wrong',
                result: null,
              });
              return;
            }
            patchState(store, { result: res.data, loading: false });
            store._usageService.loadUsage();
          },
          error: () => {
            patchState(store, {
              error: 'Server unavailable. Please try again.',
              loading: false,
            });
          },
        });
      },
      reset() {
        patchState(store, {
          loading: false,
          error: null,
          result: null,
        });
      },
    };
  }),
);
